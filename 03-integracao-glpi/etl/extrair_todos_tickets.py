...
class GLPITodosTicketsExtractor:
            ...
            while True:
                ...
                if response.status_code in [200, 206]:
                    ...
                    for user in users:
                        ...
                        realname = str(user.get('realname', '') or '').strip()
                        login = str(user.get('name', '') or '').strip()
                        if realname or firstname:
                            nome = f"{firstname} {realname}".strip()
                        elif login:
                            nome = login
                        else:
                            nome = f"Usuário {user_id}"
                        try:
                            from config import USER_OVERRIDES
                            if user_id in USER_OVERRIDES:
                                nome = USER_OVERRIDES[user_id]
                        except Exception:
                            pass
            ...
            if response.status_code in [200, 206]:
                ...
                for relation in user_relations:
                    ...
                    if ticket_id in ticket_ids:
                        nome_usuario = self.resolve_usuario_nome(user_id)
    ...
    def resolve_usuario_nome(self, user_id):
        user_id = str(user_id)
        if user_id in self.cache_usuarios:
            nome_cache = self.cache_usuarios[user_id]
            try:
                import re
                if re.fullmatch(r"Usuário \d+", nome_cache):
                    # tentar melhorar informação com consulta direta
                    pass
                else:
                    return nome_cache
            except Exception:
        ...
        try:
            ...
            r = self.session.get(url, params={'with_deleted': '1'})
            if r.status_code in [200, 206]:
                u = r.json()
                firstname = str(u.get('firstname', '') or '').strip()
                realname = str(u.get('realname', '') or '').strip()
    ...
    def extrair_todos_tickets(self):
        ...
        try:
            ...
            # Gerar nomes fixos para os arquivos (sem timestamp)
            nome_arquivo_completo = FileManager.gerar_nome_fixo('../dados/tickets_completos', 'todos_tickets')
            nome_arquivo_6m = FileManager.gerar_nome_fixo('../dados/tickets_6_meses', 'tickets_ultimos_6_meses')
            
            # Salvar arquivo completo com backup automático
            print()
            print("=" * 60)
            ...
            # Salvar arquivo dos últimos 6 meses com backup automático
            print()
            print("=" * 60)
    ...
    def enforce_base_schema(self, caminho):
        try:
            import csv, os
            cols = [
            ...
            print(f"[AVISO] Falha ao aplicar schema base: {e}")
