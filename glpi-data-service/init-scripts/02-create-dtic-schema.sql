-- ==========================================
-- Script 02: Criar Tabelas no Schema DTIC
-- ==========================================
...
-- Índices otimizados
CREATE UNIQUE INDEX idx_dtic_tickets_glpi_id ON tickets(glpi_id);
CREATE INDEX idx_dtic_tickets_status ON tickets(status);
CREATE INDEX idx_dtic_tickets_atualizado_em ON tickets(atualizado_em DESC);
CREATE INDEX idx_dtic_tickets_criado_em ON tickets(criado_em DESC);
CREATE INDEX idx_dtic_tickets_tecnico ON tickets(tecnico) WHERE tecnico IS NOT NULL;
CREATE INDEX idx_dtic_tickets_grupo ON tickets(grupo) WHERE grupo IS NOT NULL;
CREATE INDEX idx_dtic_tickets_categoria ON tickets(categoria) WHERE categoria IS NOT NULL;
CREATE INDEX idx_dtic_tickets_hash ON tickets(ticket_hash);
CREATE INDEX idx_dtic_tickets_prioridade ON tickets(prioridade);

-- Full-Text Search (PostgreSQL nativo em português)
CREATE INDEX idx_dtic_tickets_fts ON tickets 
    USING GIN(to_tsvector('portuguese', 
        coalesce(titulo, '') || ' ' || coalesce(descricao_md, '')));

-- Comentários para documentação
COMMENT ON TABLE tickets IS 'Tickets DTIC sincronizados do GLPI';
...
COMMENT ON TABLE sync_meta IS 'Metadados de sincronização (last_sync_time, etc.)';
...
CREATE INDEX idx_dtic_sync_history_iniciado ON sync_history(iniciado_em DESC);

COMMENT ON TABLE sync_history IS 'Histórico de execuções de sincronização DTIC';
...
CREATE INDEX idx_dtic_sync_errors_criado ON sync_errors(criado_em DESC);
CREATE INDEX idx_dtic_sync_errors_glpi_id ON sync_errors(glpi_id);

COMMENT ON TABLE sync_errors IS 'Log de erros durante sincronização DTIC';
...
BEGIN
    ...
    RAISE NOTICE 'Schema DTIC: % tabelas criadas', table_count;
END $$;
