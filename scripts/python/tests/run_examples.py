...
def main():
    ...
    results = {}
    for ex in [
        os.path.join('examples', 'list_followups.py'),
        os.path.join('examples', 'list_inventory.py')
    ]:
        code, out, err = run_py(ex, env)
        results[os.path.basename(ex)] = {'code': code, 'out': out.strip()[:200], 'err': err.strip()[:200]}
