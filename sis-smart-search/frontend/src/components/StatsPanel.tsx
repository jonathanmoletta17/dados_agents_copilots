export default function StatsPanel({ stats }: { stats: any }) {
  ...
  const aggregated = statusData.reduce((acc: any[], [name, count]: [string, number]) => {
    ...
    if (existing) existing.count += count; else acc.push({ label: config.label, count, color: config.color, icon: config.icon })
    return acc
  }, [])
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
      {aggregated.slice(0, 4).map((item: any, i: number) => (
        <div key={i} className="bg-white rounded-xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden border-l-4"
          style={{ borderLeftColor: item.color.includes('blue') ? '#2196F3' : item.color.includes('orange') ? '#FF9800' : item.color.includes('yellow') ? '#FFC107' : item.color.includes('green') ? '#4CAF50' : '#9E9E9E' }}>
          <div className="p-5">
            <div className="flex items-center justify-between mb-3">
              <span className="text-2xl">{item.icon}</span>
              <span className={`text-xs font-semibold px-3 py-1 rounded-full text-white ${item.color}`}>{item.count}</span>
            </div>
